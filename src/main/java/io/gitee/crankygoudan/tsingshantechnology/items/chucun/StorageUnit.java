...
public final class StorageUnit extends MenuBlock implements DistinctiveItem {
    ...
    /* Namespaced keys */
    static final NamespacedKey EMPTY_KEY = TsingshanTechnology.createKey("empty"); // key for empty item
    static final NamespacedKey DISPLAY_KEY = TsingshanTechnology.createKey("display"); // key for display item
    @Getter
    private static final NamespacedKey ITEM_KEY = TsingshanTechnology.createKey("item"); // item key for item pdc
    @Getter
    private static final NamespacedKey AMOUNT_KEY = TsingshanTechnology.createKey("stored"); // amount key for item pdc
    ...
    private static final ItemStack INTERACTION_ITEM = new CustomItemStack(Material.LIME_STAINED_GLASS_PANE,
        Y.colorRandomString ( "快捷操作"),
        Y.colorPseudorandomString("左键: 取出 1 个物品"),
        Y.colorPseudorandomString("右键: 取出 1 组物品"),
        Y.colorPseudorandomString("Shift + 左键: 全部放入")
        );
    private static final ItemStack LOADING_ITEM = new CustomItemStack(Material.CYAN_STAINED_GLASS_PANE,
        Y.colorRandomString("状态"),
        Y.colorRandomString ("加载中...")
    );
    ...
    public StorageUnit(SlimefunItemStack item, int max, ItemStack[] recipe) {
        super( C.QS_CC, item, StorageForge.TYPE, recipe);
        this.max = max;
    ...
    public static ItemMeta saveToStack(ItemMeta meta, ItemStack displayItem, String displayName, int amount) {
        if (meta.hasLore()) {
            ...
            lore.add(Y.colorRandomString("已储存: " + displayName + " x " + amount));
            meta.setLore(lore);
        }
